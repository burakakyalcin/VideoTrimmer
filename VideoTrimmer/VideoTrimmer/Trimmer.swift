...
final class Trimmer: UIControl {
    ...
    // MARK: - UIViews
    lazy var thumbView: TrimmerThumb = {
        let view = TrimmerThumb()
        view.leadingGrabber.addGestureRecognizer(leadingGestureRecognizer)
        view.trailingGrabber.addGestureRecognizer(trailingGestureRecognizer)
        view.addGestureRecognizer(thumbnailInteractionGestureRecognizer)
        return view
    }()
    ...
    private lazy var thumbnailWrapperView: UIView = {
        let view = UIView()
        view.backgroundColor = trackBackgroundColor
    ...
    private lazy var leadingThumbRest: UIView = {
        let view = UIView()
        view.backgroundColor = thumbRestColor
        view.layer.cornerRadius = 6
    ...
    private lazy var trailingThumbRest: UIView = {
        let view = UIView()
        view.backgroundColor = thumbRestColor
        view.layer.cornerRadius = 6
    ...
    private lazy var progressIndicatorControl: UIControl = {
        let control = UIControl()
        control.addGestureRecognizer(progressGestureRecognizer)
        return control
    }()
    ...
    private var range: CMTimeRange = .invalid {
        didSet {
            setNeedsLayout()
        }
    }
    ...
    private var progressIndicatorMode: ProgressIndicatorMode = .hiddenOnlyWhenTrimming {
        didSet {
    ...
    private var trackBackgroundColor = UIColor.black {
        didSet {
            thumbnailWrapperView.backgroundColor = trackBackgroundColor
        }
    }
    
    private var thumbRestColor = UIColor.black {
        didSet {
    ...
    private var visibleRange: CMTimeRange  {
    ...
    private (set) lazy var leadingGestureRecognizer: UILongPressGestureRecognizer = {
        let gestureRecognizer = UILongPressGestureRecognizer(target: self, action: #selector(leadingGrabberPanned(_:)))
        gestureRecognizer.allowableMovement = CGFloat.greatestFiniteMagnitude
        gestureRecognizer.minimumPressDuration = 0
        return gestureRecognizer
    }()
    
    private (set) lazy var trailingGestureRecognizer: UILongPressGestureRecognizer = {
        let gestureRecognizer = UILongPressGestureRecognizer(target: self, action: #selector(trailingGrabberPanned(_:)))
        gestureRecognizer.allowableMovement = CGFloat.greatestFiniteMagnitude
        gestureRecognizer.minimumPressDuration = 0
        return gestureRecognizer
    }()
    
    private (set) lazy var progressGestureRecognizer: UILongPressGestureRecognizer = {
        let gestureRecognizer = UILongPressGestureRecognizer(target: self, action: #selector(progressGrabberPanned(_:)))
        gestureRecognizer.allowableMovement = CGFloat.greatestFiniteMagnitude
        gestureRecognizer.minimumPressDuration = 0
        gestureRecognizer.require(toFail: leadingGestureRecognizer)
        gestureRecognizer.require(toFail: trailingGestureRecognizer)
        return gestureRecognizer
    }()
    
    private (set) lazy var thumbnailInteractionGestureRecognizer: UILongPressGestureRecognizer = {
        let gestureRecognizer = UILongPressGestureRecognizer(target: self, action: #selector(thumbnailPanned(_:)))
        gestureRecognizer.allowableMovement = CGFloat.greatestFiniteMagnitude
        gestureRecognizer.minimumPressDuration = 0
        gestureRecognizer.require(toFail: leadingGestureRecognizer)
        gestureRecognizer.require(toFail: trailingGestureRecognizer)
    ...
    override func layoutSubviews() {
        ...
        thumbnailTrackView.frame = CGRect(
            origin: .zero,
            size: CGSize(
                width: thumbnailRect.width - (isZoomedToEnd == false ? inset : 0),
                height: thumbnailRect.height
            )
        )
        
        thumbnailLeadingCoverView.frame = CGRect(
            x: coverStartOffset,
            y: 0,
            width: left + inset * 0.5 + coverOffset - coverStartOffset,
            height: thumbnailRect.height
        )
        
        thumbnailTrailingCoverView.frame = CGRect(
            x: right - inset * 0.5 + coverOffset,
            y: 0,
            width: thumbnailRect.width - coverStartOffset - (right - inset * 0.5 + coverOffset),
            height: thumbnailRect.height
        )
        ...
        if progressIndicator.alpha > 0 {
            ...
            let progressLeft = min(
                max(thumbView.frame.minX + inset, progressIndicatorOffset - progressWidth * 0.5),
                thumbView.frame.maxX - inset - progressWidth
            )
...
fileprivate func SnapToDevicePixels(_ rect: CGRect, scale: CGFloat? = nil) -> CGRect {
    return CGRect(
        x: SnapToDevicePixels(rect.origin.x, scale: scale),
